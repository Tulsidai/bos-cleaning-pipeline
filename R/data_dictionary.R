...
df_clean <- readRDS(file.path(p_out, "cleaned.rds"))
...
dict <- data.frame(
  variable = c("name", "author", "narrator", "time", "releasedate", "language",
               ...
               "rating_count_outlier", "price_imputed_flag"),
  ...
  label = c("Audiobook title", "Author(s)", "Narrator(s)",
            ...
            "Flag: rating count is an IQR outlier",
            "Flag: price was imputed from Free to median"),
  ...
  pct_missing_raw = c(0, 0, 0, 0, 0, 0, 82.77, 0.39, NA, NA, NA, NA, NA, NA, NA, NA),
  
  pct_missing_clean = c(0, 0, 0, 0, 0, 0, 0, 0, 82.77, 82.77, 0, 0, 0, 0, 0, 0),
  
  imputation = c(
    "None", "None", "None", "None", "None", "None", "None",
    "Median imputation — 338 Free entries replaced with median non-zero price",
  ...
  notes = c(
    ...
    "'Free' converted to median non-zero price after cleaning",
    ...
    "IQR method, k=1.5",
    "TRUE for 338 records originally priced as Free")
)
